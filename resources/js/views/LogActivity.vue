    ...
    <v-container fluid>
        ...
        <v-card>
            <v-card-text>
                ...
                <!-- TABLE -->
                <v-data-table :headers="headers" :items="logs" :loading="loading" :server-items-length="totalItems"
...
function downloadPdf() {
    const token = localStorage.getItem('token') // sesuaikan dengan tempat simpan token kamu
...
function printLogs() {
    const token = localStorage.getItem('token') // atau tempat simpan JWT-mu
    const params = new URLSearchParams({
        search: filters.value.search,
        start_date: filters.value.start_date,
        end_date: filters.value.end_date
    ...
    api.get('/logs/print?' + params, {
        ...
        responseType: 'text' // penting: karena kita ambil HTML
    }).then(res => {
